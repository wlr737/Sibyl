...
import subprocess
# from prompter_no_history import PrompterPersonet
# from finetune_ljnED_tem import Prompter_explain
from finetune_llama3_ESC import generate_prompt
...
def main(
    ...
    tokenizer = AutoTokenizer.from_pretrained("../pretrained/Meta-Llama-3-8B-Instruct/", padding_side="left")
    ...
    def save_prompt(prompt):
        open(os.path.join('/'.join(base_model.split('/')[:-1]), 'prompt_example.txt'), 'w').write(prompt)
    ...
    def evaluate(
        ...
        for prompt in prompts:
            messages, answer = generate_prompt(prompt,test=True)
    ...
    # save_path = '/'.join(base_model.split('/')[:-1]) + "/" + save_file_name
    save_list = []
