...
def generate_prompt(data_point, test=False, comet=False):
    ...
    cause = usr_last_utter['cause']
    emo = usr_last_utter['emo']
    subs = usr_last_utter['subs']
    intent = usr_last_utter['intent']
...
'''.format(cause, subs, emo, intent)
    ...
    temp = ""
    flag = 0  # 0代表上一句是usr 1代表上一句是sys
    for i, dia in enumerate(data_point['dialog']):

        if i == len(data_point['dialog']) - 1:
            output = dia['text'].strip('\n')
            messages.append({"role": "user", "content": temp})
            if not test:  # train: the model needs to learn the next ground truth response
                messages.append({"role": "assistant", "content": dia['text'].strip('\n')})
        if dia['speaker'] == 'usr' and flag == 0:
            temp += " " + dia['text'].strip('\n')
        elif dia['speaker'] == 'usr' and flag == 1:
            messages.append({"role": "assistant", "content": temp})
            flag = 0
            temp = dia['text'].strip('\n')
        elif dia['speaker'] == 'sys' and flag == 0:
            messages.append({"role": "user", "content": temp})
            flag = 1
            temp = dia['text'].strip('\n')
        elif dia['speaker'] == 'sys' and flag == 1:
            temp += " " + dia['text'].strip('\n')
